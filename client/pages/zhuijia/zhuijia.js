...
Page({
  data: {
    prizeState: 1,   //默认为1，全部，2，已中奖，3.待开奖 4.未完成
    hemaiOrder: [],  //合买订单
  ...
  onLoad: function (options) {
    ...
    wx.request({
      url: "https://nh20i6qu.qcloud.la/weapp/Myzhuijia/index",
      data: {
        openid: value,
    ...
    for (var i = 0; i < temp.length; i++) {
      if (index == i) {
        if (temp[i].select == false) {
          hemaiOrder[i] = { 'lotterytype':temp[i].lotterytype ,'shoptype': temp[i].shoptype, 'qihao': temp[i].issue, 'betnumber': temp[i].betnumber, 'username': temp[i].username, 'percent': temp[i].percent, 'select': true };
        } else {
