...
Page({
  ...
  changecolorR: function (e) {
    ...
    for (var i = 0; i < this.data.arrayRed.length; i++) {
      if (e.currentTarget.dataset.id == this.data.arrayRed[i].text) {
        ...
        //如果红球个数不小于5并且蓝球个数不小于2(可以购买至少一注)
        if (this.data.selectRed.length >= 5 && this.data.selectBlue.length >= 2) {
          var redLength = this.data.selectRed.length;
          var blueLength = this.data.selectBlue.length;
          this.data.totalZhushu = this.jiecheng(redLength) * this.jiecheng(blueLength)/ (this.jiecheng(5) * this.jiecheng(redLength - 5)*this.jiecheng(2)*this.jiecheng(blueLength-2)) ;
  ...
  changecolorB: function (e) {
    ...
    for (var i = 0; i < this.data.arrayBlue.length; i++) {
      if (e.currentTarget.dataset.id == this.data.arrayBlue[i].text) {
        ...
        //如果红球个数不小于5并且蓝球个数不小于2(可以购买至少一注)，则显示已选择投注和金额，否则清空投注和金额
        if (this.data.selectRed.length >= 5&& this.data.selectBlue.length >= 2) {
          var redLength = this.data.selectRed.length;
          var blueLength = this.data.selectBlue.length;
          this.data.totalZhushu = this.jiecheng(redLength) * this.jiecheng(blueLength) / (this.jiecheng(5) * this.jiecheng(redLength - 5) * this.jiecheng(2) * this.jiecheng(blueLength - 2));
  ...
  hemai: function () {
    if (this.data.selectRed.length >= 5 && this.data.selectBlue.length >= 2) {
      app.globalData.daletouBallsList.unshift({ "redballs": this.data.selectRed, "blueballs": this.data.selectBlue, "totalZhushu": this.data.totalZhushu, "totalMoney": this.data.totalMoney });
      wx.navigateTo({
        url: '../touzhu/touzhu?lotteryType=daletou',
      })
    } else {
      this.setData({
        warningText: "请至少选择5个红球，2个蓝球",
        warning: true
      })
    }
  },
